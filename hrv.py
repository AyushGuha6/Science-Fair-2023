...
with open("ppg_raw.csv") as raw_data:
    ...
    data = hp.filtering.filter_signal(hp.filtering.hampel_correcter(np.around(data),64),[0.7,3.5], sample_rate=64.0, order =2, filtertype="bandpass")
    plt.figure(figsize=(12,4))
    plt.plot(data)
    plt.show()
    working_data, measures = hp.process(data, 64.0, report_time=True)#, clean_rr = True)
